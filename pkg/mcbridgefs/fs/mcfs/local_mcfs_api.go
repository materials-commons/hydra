...
func (fsapi *LocalMCFSApi) Create(path string) (*mcmodel.File, error) {
	...
	if file := fsapi.knownFilesTracker.GetFile(parsedPath.FullPath()); file != nil {
...
func (fsapi *LocalMCFSApi) lookupProject(path string) (*mcmodel.File, error) {
	...
	transferRequests, err := fsapi.stors.TransferRequestStor.GetTransferRequestsForProject(parsedPath.ProjectID())
	switch {
	case err != nil:
		return nil, err
	...
	default:
		...
		f := &mcmodel.File{
			Name:      fmt.Sprintf("%d", parsedPath.ProjectID()),
			MimeType:  "directory",
			Path:      fmt.Sprintf("/%d", parsedPath.ProjectID()),
...
func (fsapi *LocalMCFSApi) lookupUser(path string) (*mcmodel.File, error) {
	...
	// user exists
	tr, err := fsapi.stors.TransferRequestStor.GetTransferRequestForProjectAndUser(parsedPath.ProjectID(), parsedPath.UserID())
	if err != nil {
		return nil, err
	}

	if tr == nil {
		return nil, fmt.Errorf("no such active user %d for project %d", parsedPath.UserID(), parsedPath.ProjectID())
	}

	f := &mcmodel.File{
		Name:     fmt.Sprintf("%d", parsedPath.UserID()),
		MimeType: "directory",
		Path:     fmt.Sprintf("/%d/%d", parsedPath.ProjectID(), parsedPath.UserID()),
...
func (fsapi *LocalMCFSApi) listActiveUsersForProject(path string) ([]mcmodel.File, error) {
	...
	inDir := &mcmodel.File{Path: fmt.Sprintf("/%d", parsedPath.ProjectID()), MimeType: "directory"}
	var dirEntries []mcmodel.File

	for _, tr := range transferRequests {
		if tr.ProjectID == parsedPath.ProjectID() {
			entry := mcmodel.File{
				Directory: inDir,
	...
	if len(dirEntries) == 0 {
		return nil, fmt.Errorf("no such project: %d", parsedPath.ProjectID())
	}
...
func (fsapi *LocalMCFSApi) listProjectDirectory(path string) ([]mcmodel.File, error) {
	...
	dir, err := fsapi.stors.FileStor.GetDirByPath(parsedPath.ProjectID(), parsedPath.ProjectPath())
	if err != nil {
		return nil, err
	}

	transferRequest, err := fsapi.stors.TransferRequestStor.
		GetTransferRequestForProjectAndUser(parsedPath.ProjectID(), parsedPath.UserID())
	if err != nil {
		return nil, err
	}
	...
	inDir := &mcmodel.File{Path: parsedPath.ProjectPath(), MimeType: "directory"}
