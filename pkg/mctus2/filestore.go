// Package mctus2 Package filestore provide a storage backend based on the local file system.
//
// LocalFileStore is a storage backend used as a handler.DataStore in handler.NewHandler.
...
type GetFilePath interface {
	GetFilePath(id string) string
}

// LocalFileStore See the handler.DataStore interface for documentation about the different
// methods.
type LocalFileStore struct {
	// Relative or absolute path to store files in. LocalFileStore does not check
...
func (store LocalFileStore) NewUpload(ctx context.Context, info handler.FileInfo) (handler.Upload, error) {
	if info.ID == "" {
		info.ID = uid()
	}
...
func (store LocalFileStore) GetUpload(ctx context.Context, id string) (handler.Upload, error) {
...
func (store LocalFileStore) AsLengthDeclarableUpload(upload handler.Upload) handler.LengthDeclarableUpload {
	return upload.(*fileUpload)
}

func (store LocalFileStore) AsConcatableUpload(upload handler.Upload) handler.ConcatableUpload {
	return upload.(*fileUpload)
}

// binPath returns the path to the file storing the binary data.
func (store LocalFileStore) binPath(id string) string {
	return filepath.Join(store.Path, id)
}

// infoPath returns the path to the .info file storing the file's info.
func (store LocalFileStore) infoPath(id string) string {
