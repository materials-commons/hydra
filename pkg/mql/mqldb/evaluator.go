...
func evalSelectProcessesAndSamples(db *DB, statement parser.Statement) ([]mcmodel.Activity, []mcmodel.Entity) {
...
// the process.
func evalSelectSamples(db *DB, statement parser.Statement) []mcmodel.Entity {
	var matchingSamples []mcmodel.Entity
	var matchingProcesses []mcmodel.Activity

	if parser.HasSampleMatchStatement(statement) {
		matchingSamples = evalMatchingSamples(db, statement)
	}

	if parser.HasProcessMatchStatement(statement) {
...
// samples.
func evalSelectProcesses(db *DB, statement parser.Statement) []mcmodel.Activity {
	var matchingProcesses []mcmodel.Activity
	var matchingSamples []mcmodel.Entity
...
func evalMatchingProcesses(db *DB, statement parser.Statement) []mcmodel.Activity {
	var matchingProcesses []mcmodel.Activity
...
func evalMatchingSamples(db *DB, statement parser.Statement) []mcmodel.Entity {
	var matchingSamples []mcmodel.Entity
...
// statements.
func eval(db *DB, process *mcmodel.Activity, sampleState *SampleState, statement parser.Statement) bool {
	switch s := statement.(type) {
	case parser.MatchStatement:
		return evalMatchStatement(db, process, sampleState, s)
	case parser.AndStatement:
		return evalAndStatement(db, process, sampleState, s)
	case parser.OrStatement:
...
// evaluates to false.
func evalAndStatement(db *DB, process *mcmodel.Activity, sampleState *SampleState, statement parser.AndStatement) bool {
...
// to true.
func evalOrStatement(db *DB, process *mcmodel.Activity, sampleState *SampleState, statement parser.OrStatement) bool {
	if eval(db, process, sampleState, statement.Left) {
		return true
	}
...
func evalMatchStatement(db *DB, process *mcmodel.Activity, sampleState *SampleState, match parser.MatchStatement) bool {
	...
	case parser.ProcessFieldType:
		return evalProcessFieldMatch(process, match)
	case parser.ProcessAttributeFieldType:
	...
	case parser.SampleFieldType:
		return evalSampleFieldMatch(sampleState, match)
	case parser.SampleAttributeFieldType:
	...
	case parser.ProcessFuncType:
		return evalProcessFuncMatch(process, db, match)
	case parser.SampleFuncType:
...
func evalProcessFuncMatch(process *mcmodel.Activity, db *DB, match parser.MatchStatement) bool {
	if process == nil {
		return false
	}
...
func evalProcessAttributeFieldMatchForSampleState(sampleState *SampleState, db *DB, match parser.MatchStatement) bool {
...
// that attribute.
func evalProcessAttributeFieldMatch(process *mcmodel.Activity, db *DB, match parser.MatchStatement) bool {
	if process == nil {
...
func evalSampleAttributeFieldMatchForProcess(process *mcmodel.Activity, db *DB, match parser.MatchStatement) bool {
...
func evalSampleAttributeFieldMatch(sampleState *SampleState, db *DB, match parser.MatchStatement) bool {
	// Sanity check, make sure sampleState isn't nil
	if sampleState == nil {
		return false
	}
